...
async function makeRequests() {
    fillManagersTable().then(function () {
        fillDevelopersTable().then(function () {
            fillSelect('../api/issues/ListRol', 'id_rol', null).then(function () {
                fillSelect('../api/issues/ListStatus', 'id_status', null).then(function () {
...
document.getElementById('manageUsers-form').addEventListener('submit', function (event) {
    ...
    if (document.getElementById('password').value == document.getElementById('password2').value) {
        ...
        if (document.getElementById('id_user').value != '') {
            ...
            saveOrUpdateData('../api/issues/create', data, 'manageUsersModal').then(function () {
                makeRequests();
            });
        }
    } else {
